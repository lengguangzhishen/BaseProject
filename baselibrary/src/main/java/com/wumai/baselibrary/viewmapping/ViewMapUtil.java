package com.wumai.baselibrary.viewmapping;
...
public final class ViewMapUtil {
    ...
    public static <T> Pair<T, View> map(Class<T> clazz,
        ...
        try {
            ...
            final ViewMapping viewMapping = getViewMapping(clazz);
            if (viewMapping == null) {
                return null;
            }
            View rootView = inflater.inflate(viewMapping.value(), root, false);
            map(object, rootView);
            return Pair.create(object, rootView);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public static <T> Pair<T, View> mapForConvert(Class<T> clazz,
        ...
        return new Pair<>($t, convertView);
    }

   public static ViewMapping getViewMapping(Class<?> clazz) {
        ViewMapping mapping;
        while ((!Activity.class.equals(clazz))&& clazz != null) {
