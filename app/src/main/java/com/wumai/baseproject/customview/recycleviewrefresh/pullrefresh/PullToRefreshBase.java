 ...
 *******************************************************************************/
package com.wumai.baseproject.customview.recycleviewrefresh.pullrefresh;
...
import com.wumai.baseproject.customview.recycleviewrefresh.pullrefresh.internal.FlipLoadingLayout;
import com.wumai.baseproject.customview.recycleviewrefresh.pullrefresh.internal.LoadingLayout;
import com.wumai.baseproject.customview.recycleviewrefresh.pullrefresh.internal.RotateLoadingLayout;
import com.wumai.baseproject.customview.recycleviewrefresh.pullrefresh.internal.Utils;
import com.wumai.baseproject.customview.recycleviewrefresh.pullrefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    static final boolean DEBUG = BaseProjectApplication.getInstance().isDebugMode();
...
//				}
                ...
                if (!mScrollingWhileRefreshingEnabled && isRefreshing()) {
                    onRefreshComplete();//此处主要修改下拉刷新再次向上滑动headerview的隐藏
                    return false;
                }
    ...
    public void showNoMoreData(boolean flag) {
        if (!flag) {
            ...
            if (mNoMoreTextView != null) {
                this.removeView(mNoMoreTextView);
            }
            return;
        }
        if (mNoMoreTextView == null) {
            mNoMoreTextView = new TextView(getContext());
            mNoMoreTextView.setText("没有更多内容");
            mNoMoreTextView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 14);
            int height = DensityUtil.dip2px(50);
            LayoutParams lp = new LayoutParams(LayoutParams.MATCH_PARENT, height);
            mNoMoreTextView.setLayoutParams(lp);
        }
        if (mFooterLayout != null) {
            this.removeView(mFooterLayout);
        }
        if (flag) {
            this.addView(mNoMoreTextView);
        }
    }
}
